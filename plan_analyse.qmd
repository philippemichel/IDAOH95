---
subtitle: "Plan d'analyse statistique V1.11"
lof: false
lot: false
---
"
...
Vu le faible nombre de cas on ne fera pas d'hypothèse de normalité. Les variables numériques seront présentées par leur \gls{mediane} avec les \gls{quartiles} & comparées grâce au test \gls{nonparam} de Wilcoxon pour séries non appariées. Les \gls{discrete} seront présentées en nombre avec le pourcentage. L'intervalle de confiance des pourcentages (à 95 %) sera calculé par \gls{bootstrap} (package `boot` [@boot]). Le test du $\chi^2$ de Spearman sera utilisé sous réserve d'un effectif suffisant, à défaut le test exact de Fischer. Des graphiques seront réalisés pour les résultats importants (package `ggplot2` [@ggplot]).

Pour les objectifs où la même question est posée à plusieurs professions, une analyse recherchera une éventuelle corrélation entre les réponses & l’établissement par analyse en régression logistique (objectif principal, objectifs secondaires 4 & 5).   

## Taille de l'échantillon

L'unité est l'\gls{ehpad} & il y a 75 établissements dans le département qui peuvent potentiellement participer à l’enquête.
...
*Les principales missions pouvant être déléguées à des \gls{ipa} seront identifiées par auto-questionnaire des médecins coordonnateurs (question n°19) et des \gls{idec} (question n°15).*
...
Les items proposés seront classés par les participants. On attribuera un rang de 1 à 10 pour chaque item. Les résultats seront présentés en \gls{mediane} avec les \gls{quartiles} avec un diagramme en boxplot ou en Ridgeline (package `ggridges` [@ggridges]) comparant les réponses.
...
### Objectif 5
*Les directeurs (question n°17), médecins coordonnateurs (question n°22) et \gls{idec} (question n°17) seront sollicités pour identifier des axes d’améliorations organisationnels.*

La même" liste de réponses est proposée aux trois groupes professionnels. Les items proposés seront classés par les participants. On attribuera un rang de 1 à 10 pour chaque item. Les résultats seront présentés en \gls{mediane} avec les \gls{quartiles} avec un diagramme en boxplot ou en Ridgeline (package `ggridges` [@ggridges]) comparant les réponses pour chaque groupe puis en comparant les réponses entre les trois groupes.
