...
zza <- rbind(ipad,ipai,ipam) |> 
  dplyr::filter(missions_ipa != "") |> 
group_by(id, profession) |> 
  summarise(nrep =n())

var_label(zza$nrep) <-  "Nombre de réponses"
  
zza |> 
  dplyr::select(!id) |> 
  tbl_summary(by = profession,
              include = c(profession, nrep)) |>
  add_p() |> 
  bold_labels() |>
  modify_header(label ~ " ") |>
    gexptabph(exp = expx,
            nomfich = classeur ,
            nomsheet = "nb_case_prof") 
...
aa <- zz |> 
group_by(id, nb_lits_rec) |> 
  summarise(nrep =n())

var_label(aa$nrep) <-  "Nombre de réponses"
  
aa |> 
  dplyr::select(!id) |> 
  tbl_summary(by = nb_lits_rec,
              include = c(nb_lits_rec, nrep)) |>
  add_p() |> 
  bold_labels() |>
  modify_header(label ~ " ") |>
    gexptabph(exp = expx,
            nomfich = classeur ,
            nomsheet = "nb_case_taille") 
```
...
# Technique 
 % 
Un risque $\alpha$ de 5 % & une puissance de 80 % ont été retenus pour les tests statistiques.

Vu la faible taille de l'échantillon aucune hypothèse de normalité ne peut être effectuée. Les données numériques sont donc présentées en médianes avec les quartiles & comparées par des tests non paramétriques (test de Wilcoxon pour séries non appariées ou test de Kruskal-Wallis si plus de deux modalités). LEs données discrètes sont présentées en nombre & pourcentage & comparées par des tests du $\Chi^2$ de Pearson.

L'analyse statistique a été réalisée avec le logiciel **R** (R Core Team (2025) -- Vienna)
